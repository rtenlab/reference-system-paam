...
bool chainset::schedulable(void)
{
    ...
    // Sort executors in descending priority order
    /*
...
void chainset::request_driven_gpu_bound(void)
{
    ...
    for (auto &callback : this->callbacks)
    {
        ...
        // Find the maximum GPU execution time of all other callbacks with higher priority
        for (auto &other_callback : this->callbacks)
        { // If the other callback has higher priority and is not in the same chain and is in the same bucket
            if (other_callback->priority < callback->priority && other_callback->chain_id != callback->chain_id && other_callback->bucket == callback->bucket && other_callback->G + other_callback->epsilon > max_gpu_exe)
            {
        ...
        while (true)
        {
            ...
            for (auto &selected_callback : this->callbacks)
            {
                ...
                if (this->chains.at(selected_callback->chain_id)->sem_priority > this->chains.at(callback->chain_id)->sem_priority && selected_callback->id != callback->id && selected_callback->chain_id != callback->chain_id)
                {
...
// double chainset::job_driven_gpu_bound(callback *t_callback, std::vector<callback> chain_callbacks, double R)
double chainset::job_driven_gpu_bound(std::shared_ptr<callback> t_callback, std::vector<std::shared_ptr<callback>> chain_callbacks, double R)
{
    ...
    for (auto &callback : chain_callbacks)
    {
        if (callback->priority < t_callback->priority && callback->chain_id != t_callback->chain_id &&
            callback->bucket == t_callback->bucket && callback->G + callback->epsilon > max_gpu_exe)
        {
    ...
    for (auto &callback : chain_callbacks)
    {
        if (this->chains.at(callback->chain_id)->sem_priority > this->chains.at(t_callback->chain_id)->sem_priority &&
            callback->id != t_callback->id && callback->chain_id != t_callback->chain_id)
        {
