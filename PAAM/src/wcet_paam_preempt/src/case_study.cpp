...
public:
      ...
      : Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(USE_INTRA_PROCESS_COMMS)), count_(0), trace_callbacks_(trace_ptr), exe_time_(exe_time), period_(period), end_flag_(end_flag), callback_priority_(callback_priority), kernel_id_(kernel_id), gemm_size(gemm_size)
  {
    ...
    this->request_publisher_ = this->create_publisher<paam_server_interfaces::msg::GPURequest>("request_topic", 10);
    this->reg_publisher_ = this->create_publisher<paam_server_interfaces::msg::GPURegister>("registration_topic", 10);
    std::this_thread::sleep_for(7000ms);
    this->register_sub_ = this->create_subscription<paam_server_interfaces::msg::GPURegister>(
  ...
  void paam_hist_wrapper(int size, int chain_priority, bool sleep)
  {
  ...
  void paam_tpu_wrapper(int size, int chain_priority, bool sleep)
  {
    if (!this->handshake_complete)
    {
      RCLCPP_INFO(this->get_logger(), "Handshake Not Complete");
  ...
  void paam_gemm_wrapper(int size, int chain_priority, bool sleep)
  {
    if (!this->handshake_complete)
    {
      RCLCPP_INFO(this->get_logger(), "Handshake Not Complete");
  ...
  void paam_red_wrapper(int size, int chain_priority, bool sleep)
  {
    if (!this->handshake_complete)
    {
      RCLCPP_INFO(this->get_logger(), "Handshake Not Complete");
  ...
  void paam_vec_wrapper(int size, int chain_priority, bool sleep)
  {
    if (!this->handshake_complete)
    {
      RCLCPP_INFO(this->get_logger(), "Handshake Not Complete");
  ...
  void paam_yolo_wrapper(int size, int chain_priority, bool sleep)
  {
    if (!this->handshake_complete)
    {
      RCLCPP_INFO(this->get_logger(), "Handshake Not Complete");
  ...
  void paam_lane_wrapper(int size, int chain_priority, bool sleep)
  {
    if (!this->handshake_complete)
    {
      RCLCPP_INFO(this->get_logger(), "Handshake Not Complete");
  ...
  void send_handshake(std::string callback_name, int chain_priority)
  {
    ...
    paam_server_interfaces::msg::GPURegister message;
...
private:
  ...
  int kernel_id_;
  cv::VideoCapture capture; //('/home/paam/Documents/sample.mp4');
  void load_class_list()
  {
    std::ifstream ifs("/home/paam/Documents/ros2_gpu_server/src/gpu_responder/net/classes.txt");
    std::string line;
  ...
  void handshake_callback(paam_server_interfaces::msg::GPURegister::SharedPtr request)
  {
  ...
  void make_requests()
  {
    while (rclcpp::ok())
    {
      ...
      {
      case 0:
        this->paam_hist_wrapper(1000000, callback_priority_, true);

        break;
      case 1:
        this->paam_gemm_wrapper(1000000, callback_priority_, true);

        break;
      case 2:
        // this->make_yolo_goal(this->yolo_shm);
        // this->paam_yolo_wrapper(1000000, callback_priority_, true);
        // this->yolo_verify(this->yolo_shm);
        break;
      case 3:
        this->paam_red_wrapper(1000000, callback_priority_, true);
        break;
      case 4:
        this->paam_vec_wrapper(1000000, callback_priority_, true);
        break;
      case 5:
        this->paam_tpu_wrapper(1000000, callback_priority_, true);
        break;
      default:
        break;
      }
  ...
  void write_to_shm()
  {
    ...
    // this->capture.open("/home/paam/Research/sample.mp4");
    // this->make_yolo_goal(this->yolo_shm);
  ...
  void populateLoanedGEMMMessage(rclcpp::LoanedMessage<paam_server_interfaces::msg::GPURequest> &loanedMsg, int chain_priority)
  {
  ...
  {
    auto hist_message = paam_server_interfaces::msg::GPURequest();
  ...
  {
    auto vec_message = paam_server_interfaces::msg::GPURequest();
  ...
  {
    auto red_message = paam_server_interfaces::msg::GPURequest();
  ...
  {
    auto yolo_message = paam_server_interfaces::msg::GPURequest();
  ...
  void populateLoanedTPUMessage(rclcpp::LoanedMessage<paam_server_interfaces::msg::GPURequest> &loanedMsg, int chain_priority)
  {
  ...
  {
    auto lane_message = paam_server_interfaces::msg::GPURequest();
  ...
  std::string label_file = "/home/paam/Research/paam-RTAS/src/paam_server/test_data/imagenet_labels.1.txt";
...
int main(int argc, char *argv[])
{
  ...
  timeval ctime;
  gettimeofday(&ctime, NULL);
  std::shared_ptr<trace::Trace> trace_callbacks = std::make_shared<trace::Trace>("/home/paam/Research/data/wcet_t1_" + kernel + "_paam.txt");
  ...
  std::shared_ptr<trace::Trace> trace_callbacks2 = std::make_shared<trace::Trace>("/home/paam/Research/data/wcet_t2_" + kernel + "_paam.txt");
